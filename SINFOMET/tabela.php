...
<!-- Inicio da tabela em PHP-->
    ...
    $sql = "SELECT * FROM sinfomet WHERE dat_coleta BETWEEN '$dado' AND '$dado1' ORDER BY StampTime ASC;";
    ...
    if ($buscar->num_rows > 0) {
        ...
        echo '<th width="5%">' . "BattV"    . '<sub>' . "(Volts)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "TEMP"   . '<sub>' . "(C)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "PTS"   . '<sub>' . "(&#181g/m3)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "PM10"   . '<sub>' . "(&#181g/m3)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "PM2_5"   . '<sub>' . "(&#181g/m3)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "PM1"  . '<sub>' . "(&#181g/m3)" . '</sub>' . '</th>';
        echo '</tr>';
        echo '</thead>';
    }else{
    ...
    while ($dados = mysqli_fetch_array($buscar)) {
        ...
        $BATTV = $dados['val_coleta2'];
        $TEMP = $dados['val_coleta3'];
        $PTS = $dados['val_coleta4'];
        $PM10 = $dados['val_coleta5'];
        $PM25 = $dados['val_coleta6'];
        $PM1 = $dados['val_coleta7'];
