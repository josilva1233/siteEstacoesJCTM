...
<body>
    ...
    $sql = "SELECT * FROM cimentosliz WHERE dat_coleta BETWEEN '$dado' AND '$dado1' ORDER BY StampTime ASC";
    ...
    if ($buscar->num_rows > 0) {
        ...
        echo '<th width="5%">' . "BP"   . '<sub>' . "(mbr)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "SR"   . '<sub>' . "(W/m2)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "WS"   . '<sub>' . "(m/s)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "WD"   . '<sub>' . "(m/s)" . '</sub>' . '</th>';
        echo '<th width="5%">' . "PP"   . '<sub>' . "(mm)" . '</sub>' . '</th>';
        echo '</tr>';
        echo '</thead>';
    } else {
    ...
    while ($dados = mysqli_fetch_array($buscar)) {
        ...
        $PM25 = $dados['val_coleta8'];
        $PM10 = $dados['val_coleta9'];
        $AT = $dados['val_coleta10'];
        $RH = $dados['val_coleta11'];
        $BP = $dados['val_coleta12'];
        $SR = $dados['val_coleta13'];
        $WS = $dados['val_coleta14'];
        $WD = $dados['val_coleta15'];
        $PP = $dados['val_coleta16'];
